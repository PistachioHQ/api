            ...
            crate::generated_admin::apis::Error::ResponseError(resp) => {
                ...
                if let Some(problem) = parse_error_details(&resp.content) {
                    return match status {
                        ...
                        500..=599 => {
                            ListTenantsError::ServiceError(problem.message.unwrap_or(problem.title))
                        }
                ...
                match status {
                    400 => ListTenantsError::BadRequest(fallback_error_details(resp.content)),
                    401 => ListTenantsError::Unauthenticated(resp.content),
                    403 => ListTenantsError::PermissionDenied(resp.content),
                    404 => ListTenantsError::NotFound(fallback_error_details(resp.content)),
