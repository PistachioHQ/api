            ...
            crate::generated_admin::apis::Error::ResponseError(resp) => {
                ...
                if let Some(problem) = parse_problem_details(&resp.content, status) {
                    return match status {
                        ...
                        401 => {
                            ListAppsError::Unauthenticated(problem.detail.unwrap_or(problem.title))
                        }
                        403 => {
                            ListAppsError::PermissionDenied(problem.detail.unwrap_or(problem.title))
                        }
                        404 => ListAppsError::NotFound(problem),
                        500..=599 => {
                            ListAppsError::ServiceError(problem.detail.unwrap_or(problem.title))
                        }
                ...
                match status {
                    400 => ListAppsError::BadRequest(fallback_problem_details(400, resp.content)),
                    401 => ListAppsError::Unauthenticated(resp.content),
                    403 => ListAppsError::PermissionDenied(resp.content),
                    404 => ListAppsError::NotFound(fallback_problem_details(404, resp.content)),
