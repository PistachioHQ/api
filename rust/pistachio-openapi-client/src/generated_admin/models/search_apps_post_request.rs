/*
 * Pistachio Admin API
 *
 * Administrative API for managing Pistachio projects and resources. Requires service account authentication.
 *
 * The version of the OpenAPI document: 0.1.0
 *
 * Generated by: https://openapi-generator.tech
 */

use crate::generated_admin::models;
use serde::{Deserialize, Serialize};

/// SearchAppsPostRequest : Request body for searching apps within a project.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct SearchAppsPostRequest {
    #[serde(rename = "params", skip_serializing_if = "Option::is_none")]
    pub params: Option<Box<models::SearchTenantsPostRequestParams>>,
}

impl SearchAppsPostRequest {
    /// Request body for searching apps within a project.
    pub fn new() -> SearchAppsPostRequest {
        SearchAppsPostRequest { params: None }
    }
}
